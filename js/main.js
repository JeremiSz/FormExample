...
//ANSWER SYSTEM
{
  ...
  const eleSections = document.getElementsByClassName("section");
  ...
  //SETUP
  {
    ...
    for (let i = 0; i < sizes.length; i++) {
      answers.push(Array(sizes[i]));
      filled.push(Array(sizes[i]));
    }
    ...
    for (let i = 1; i < eleSections.length; i++) {
      eleSections[i].style.display = "none"
    }
    ...
    //attaching html
    {
          ...
          for (const element of elements) {
            if (element.type === "checkbox") {
              element.addEventListener("click", () => {
                addAnswer(section, question, this.value, this.checked)
              });
            }
            else {
              element.addEventListener("click", () => {
                setAnswer(section, question, this.value)
              });
            }
          }
        }
      ...
      //setting revealers
      {
        let S4Q1 = document.getElementsByName("S4Q1");
        const optional4 = document.getElementById("optional4")

        S4Q1[0].addEventListener("click", () => optional4.style.display = "block");
        S4Q1[1].addEventListener("click", () => optional4.style.display = "none");
      }
    }
  }
  ...
  function _showResult(section){
    ...
    let grade = Number(_getGrade(section));
  ...
  function _revealNext(section){
    if(section < eleSections.length)
      eleSections[section].style.display = "block";
  }
}
